...
const obrasRouter = router({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!rl.allowed)
        throw new TRPCError({
          ...
          message: `Limite de criação atingido. Tente em ${rl.retryAfterSec}s.`,
        });
  ...
  approve: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logAdm({
        adminId: ctx.user.id,
        acao: input.status === "aprovada" ? "aprovar_obra" : "rejeitar_obra",
        targetType: "obra",
        targetId: input.id,
      });
      return { success: true };
...
const capitulosRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!rl.allowed)
        throw new TRPCError({
          code: "TOO_MANY_REQUESTS",
          message: `Limite de capítulos atingido. Tente em ${rl.retryAfterSec}s.`,
        });
      ...
      if (ctx.user.role === "tradutor_aprendiz") {
        ...
        if (aguardando >= 10)
          throw new TRPCError({
            code: "FORBIDDEN",
            message:
              "Você já tem 10 capítulos aguardando aprovação. Aguarde antes de enviar mais.",
          });
      }
  ...
  approve: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logAdm({
        adminId: ctx.user.id,
        acao: input.status === "aprovado" ? "aprovar_capitulo" : "rejeitar_capitulo",
        targetType: "capitulo",
        targetId: input.id,
      });
      return { success: true };
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!rl.allowed)
        throw new TRPCError({
          code: "TOO_MANY_REQUESTS",
          message: `Limite de denúncias atingido. Aguarde ${rl.retryAfterSec}s.`,
        });
...
const pedidoCargoRouter = router({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (ctx.user.ultimoPedidoCargo) {
        ...
        if (diasPassados < 10) {
          ...
          throw new TRPCError({
            code: "TOO_MANY_REQUESTS",
            message: `Aguarde ${diasRestantes} dia(s) antes de fazer um novo pedido.`,
          });
        }
      }
  ...
  meuPedidoRecente: protectedProcedure.query(async ({ ctx }) => {
    ...
    const bloqueadoAte = ctx.user.ultimoPedidoCargo
      ? new Date(
          new Date(ctx.user.ultimoPedidoCargo).getTime() + 10 * 24 * 60 * 60 * 1000
        )
      : null;
...
const adminRouter = router({
  ...
  setRole: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.role === "admin_supremo")
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "O cargo de Admin Supremo não pode ser atribuído por aqui.",
        });
  ...
  listReports: protectedProcedure
    .input(
      z.object({ page: z.number().optional(), resolved: z.boolean().optional() })
    )
    .query(({ ctx, input }) => {
      if (!isAdmin(ctx.user.role)) throw new TRPCError({ code: "FORBIDDEN" });
