...
function isSupremeAdmin(role: string) {
return role === "admin_supremo";
}
function isTranslatorOrAbove(role: string) {
return roleLevel(role) >= 1;
}
function isOfficialOrAbove(role: string) {
return roleLevel(role) >= 2;
}
// ✅ Verifica se usuário pode interagir (banimento suave bloqueia
interações)
...
throw new TRPCError({
code: "FORBIDDEN",
message: "Você está impedido de interagir no momento.",
});
}
// ─── Obras Router
─────────────────────────────────────────────────────────────
...
limit: z.number().int().min(1).max(50).optional(), // ✅ máximo 50
por requisição
})
)
...
.input(
z.object({
status: z.enum(["em_espera", "aprovada", "rejeitada"]).optional(),
...
if (!isAuthor && !isAdminUser) throw new TRPCError({ code:
"NOT_FOUND" });
}
return obra;
}),
...
if (!isTranslatorOrAbove(ctx.user.role))
throw new TRPCError({
code: "FORBIDDEN",
message: "Apenas tradutores podem criar obras.",
});
// ✅ Rate limit: 3 obras por dia
const rl = checkRateLimit({
key: `criarObra:${ctx.user.id}`,
...LIMITS.criarObra,
});
if (!rl.allowed)
throw new TRPCError({
code: "TOO_MANY_REQUESTS",
message: `Limite de criação atingido. Tente em
${rl.retryAfterSec}s.`,
});
const status = isOfficialOrAbove(ctx.user.role) ? "aprovada" :
"em_espera";
await createObra({ ...input, authorId: ctx.user.id, status });
return { success: true, status };
}),
approve: protectedProcedure
.input(z.object({ id: z.number(), status: z.enum(["aprovada",
"rejeitada"]) }))
.mutation(async ({ ctx, input }) => {
if (!isAdmin(ctx.user.role)) throw new TRPCError({ code: "FORBIDDEN"
});
...
await logAdm({
adminId: ctx.user.id,
acao: input.status === "aprovada" ? "aprovar_obra" :
"rejeitar_obra",
targetType: "obra",
targetId: input.id,
});
...
if (!isSupremeAdmin(ctx.user.role))
throw new TRPCError({
code: "FORBIDDEN",
message: "Apenas o Admin Supremo pode transferir autoria.",
});
await updateObraAuthor(input.obraId, input.newAuthorId);
await logAdm({
adminId: ctx.user.id,
acao: "alterar_author_obra",
detalhes: `Novo authorId: ${input.newAuthorId}`,
targetType: "obra",
targetId: input.obraId,
});
return { success: true };
}),
...
input.includeAll &&
ctx.user &&
(isAdmin(ctx.user.role) || isTranslatorOrAbove(ctx.user.role));
...
if (!obra) throw new TRPCError({ code: "NOT_FOUND" });
if (obra.authorId !== ctx.user.id && !isAdmin(ctx.user.role))
throw new TRPCError({
code: "FORBIDDEN",
message: "Apenas o autor da obra pode adicionar capítulos.",
});
// ✅ Rate limit: 10 capítulos por hora
const rl = checkRateLimit({
key: `criarCap:${ctx.user.id}`,
...LIMITS.criarCapitulo,
});
if (!rl.allowed)
throw new TRPCError({
code: "TOO_MANY_REQUESTS",
message: `Limite de capítulos atingido. Tente em
${rl.retryAfterSec}s.`,
});
...
code: "FORBIDDEN",
message:
"Você já tem 10 capítulos aguardando aprovação. Aguarde antes
de enviar mais.",
});
}
const status = isOfficialOrAbove(ctx.user.role) ? "aprovado" :
"aguardando";
...
approve: protectedProcedure
.input(z.object({ id: z.number(), status: z.enum(["aprovado",
"rejeitado"]) }))
.mutation(async ({ ctx, input }) => {
if (!isAdmin(ctx.user.role)) throw new TRPCError({ code: "FORBIDDEN"
});
...
await logAdm({
adminId: ctx.user.id,
acao: input.status === "aprovado" ? "aprovar_capitulo" :
"rejeitar_capitulo",
targetType: "capitulo",
targetId: input.id,
});
...
.query(({ input }) => listComentarios(input.obraId)),
create: protectedProcedure
.input(z.object({ obraId: z.number(), content:
z.string().min(1).max(500) }))
.mutation(async ({ ctx, input }) => {
canInteract(ctx.user);
// ✅ Rate limit: 10 comentários por minuto
const rl = checkRateLimit({
key: `comentario:${ctx.user.id}`,
...LIMITS.comentario,
});
if (!rl.allowed)
throw new TRPCError({
code: "TOO_MANY_REQUESTS",
message: `Muitos comentários. Aguarde ${rl.retryAfterSec}s.`,
});
...
.input(z.object({ obraId: z.number() }))
.query(({ input }) => countCurtidas(input.obraId)),
status: protectedProcedure
.input(z.object({ obraId: z.number() }))
.query(({ ctx, input }) =>
getCurtida(input.obraId, ctx.user.id).then((r) => !!r)
),
...
.input(z.object({ obraId: z.number() }))
.query(({ ctx, input }) =>
getFavorito(ctx.user.id, input.obraId).then((r) => !!r)
),
toggle: protectedProcedure
.input(z.object({ obraId: z.number() }))
.mutation(({ ctx, input }) => {
...
getHistoricoLeitura(ctx.user.id)
),
...
.mutation(({ ctx, input }) =>
upsertHistoricoLeitura({ ...input, userId: ctx.user.id })
),
});
// ─── Reports Router
───────────────────────────────────────────────────────────
...
.input(
z.object({
tipo: z.enum(["quero_aprender", "posso_ajudar"]),
mensagem: z.string().max(500).optional(),
})
)
.mutation(async ({ ctx, input }) => {
if (ctx.user.role !== "usuario")
throw new TRPCError({
code: "FORBIDDEN",
message: "Apenas usuários comuns podem solicitar cargo.",
});
if (ctx.user.bannedTotal || ctx.user.banned)
throw new TRPCError({
code: "FORBIDDEN",
...
const diasPassados =
(Date.now() - new Date(ctx.user.ultimoPedidoCargo).getTime()) /
(1000 * 60 * 60 * 24);
if (diasPassados < 10) {
const diasRestantes = Math.ceil(10 - diasPassados);
throw new TRPCError({
code: "TOO_MANY_REQUESTS",
message: `Aguarde ${diasRestantes} dia(s) antes de fazer um novo
pedido.`,
});
}
}
await criarPedidoCargo({
userId: ctx.user.id,
tipo: input.tipo,
...
? new Date(
new Date(ctx.user.ultimoPedidoCargo).getTime() + 10 * 24 * 60 * 60
* 1000
)
: null;
...
listNotificacoes(ctx.user.id)),
countNaoLidas: protectedProcedure.query(({ ctx }) =>
countNotificacoesNaoLidas(ctx.user.id)
),
marcarLida: protectedProcedure
.input(z.object({ id: z.number() }))
.mutation(({ ctx, input }) => marcarNotificacaoLida(input.id,
ctx.user.id)),
...
notifs
.filter((n: any) => !n.lida)
.map((n: any) => marcarNotificacaoLida(n.id, ctx.user.id))
);
return { success: true };
}),
});
...
if (
!isSupremeAdmin(ctx.user.role) &&
(input.role === "admin" || input.role === "admin_supremo")
)
throw new TRPCError({
code: "FORBIDDEN",
message: "Apenas Admin Supremo pode promover a Admin.",
});
if (input.role === "admin_supremo")
throw new TRPCError({
code: "FORBIDDEN",
message: "O cargo de Admin Supremo não pode ser atribuído por
aqui.",
});
...
if (alvo && isSupremeAdmin(alvo.role))
throw new TRPCError({
code: "FORBIDDEN",
message: "O Admin Supremo não pode ser banido.",
});
...
.mutation(async ({ ctx, input }) => {
// ✅ Só Admin Supremo pode editar links públicos (Discord, Telegram,
etc)
if (!isSupremeAdmin(ctx.user.role))
throw new TRPCError({
code: "FORBIDDEN",
message: "Apenas o Admin Supremo pode editar links públicos.",
});
...
.input(
z.object({ page: z.number().optional(), resolved:
z.boolean().optional() })
)
.query(({ ctx, input }) => {
if (!isAdmin(ctx.user.role)) throw new TRPCError({ code: "FORBIDDEN"
...
resposta: z.string().max(500).optional(),
})
)
.mutation(async ({ ctx, input }) => {
if (!isAdmin(ctx.user.role)) throw new TRPCError({ code: "FORBIDDEN"
});
await avaliarPedidoCargo({
...
await logAdm({
adminId: ctx.user.id,
acao: `pedido_cargo_${input.status}`,
targetType: "usuario",
targetId: input.pedidoId,
});
return { success: true };
}),
});
